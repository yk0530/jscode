...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState({
        categoryView:state => state.detail.goodInfo.categoryView || {},
      }),
      ...mapGetters(['spuSaleAttrList','skuInfo'])
    },
    methods:{
      ...
      async addShopCar(){
        try {
          let result = await this.$store.dispatch('getAddOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
